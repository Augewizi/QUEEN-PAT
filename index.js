...
global.loadDatabase = async function loadDatabase() {
  ...
  try {
    ...
    if (!global.db.data || Object.keys(global.db.data).length === 0) {
      console.log("[PAT-V2] Syncing local database...");
      await readDB();
      await global.db.read();
    }
  } catch (_0x363664) {
...
async function createDB() {
  ...
  try {
    ...
    console.log("[PAT-V2] Database created successfully.");
  } catch (_0xdf529d) {
...
async function readDB() {
  ...
  try {
    ...
    console.log("[?PAT-V2?] Fetching database from GitHub...");
    const _0x518cb2 = {
      prefix: ".",
      mode: "public",
      autobio: false,
    ...
    console.log("[?PAT-V2?] Local database updated.");
  } catch (_0x3056d0) {
...
async function checkForUpdates() {
  try {
    const _0x12847e = "queenadiza";
    const _0x460d7a = "Adiza-bot";
    const _0x3b1023 = "https://api.github.com/repos/" + _0x12847e + "/" + _0x460d7a + "/releases/latest";
    const _0xf9b1c5 = await axios.get(_0x3b1023);
    const _0x4b267f = _0xf9b1c5.data.tag_name.replace("v", "");
    if (_0x4b267f !== versions) {
      return "🚀 *Update available!*\n┃ » Latest: v" + _0x4b267f + "\n┃ » Current: v" + versions;
    } else {
      return "✅ Aidza-bot is up to date (v" + versions + ").";
    }
  } catch (_0x522b55) {
    return " ⚠️ Failed to check for updates.";
  }
}
function cleanUp() {
...
async function startCypher() {
  ...
  if (usePairingCode && !_0x5e5430.authState.creds.registered) {
    ...
    let _0xedecbe;
    _0xedecbe = await question(chalk.bgBlack(chalk.greenBright("Number to be connected to Adiza Bot?\nExample 233593734312:- ")));
    ...
    try {
      ...
      if (_0x54cce8.connection == "open") {
        ...
        try {
          ...
          console.log("Failed to join the group: " + (_0x9d07d0.message || _0x9d07d0));
